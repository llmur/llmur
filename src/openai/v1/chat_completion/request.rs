...
pub struct ChatCompletionRequest {
	...
	/// verbatim. [See more information about frequency and presence penalties](https://platform.openai.com/docs/guides/text-generation/frequency-and-presence-penalties).
	#[cfg_attr(feature = "serde", serde(skip_serializing_if = "Option::is_none"))]
	...
	/// appear in the text so far, increasing the model's likelihood to talk about new topics. [See more information about frequency and presence penalties](https://platform.openai.com/docs/guides/text-generation/frequency-and-presence-penalties).
	#[cfg_attr(feature = "serde", serde(skip_serializing_if = "Option::is_none"))]
	...
	/// model generates is valid JSON. **Important**: when using JSON mode, you **must** also
	/// instruct the model to produce JSON yourself via a system or user message. Without this, the
	/// model may generate an unending stream of whitespace until the generation reaches the token
	/// limit, resulting in a long-running and seemingly "stuck" request. Also note that the
	/// message content may be partially cut off if `finish_reason="length"`, which indicates the
	...
	/// or more tools. Specifying a particular tool via `{"type": "function", "function": {"name":
...
mod tests {
	...
	fn test_chat_completions_01_decode_ok() -> Result<()> {
		...
		}
		{
			...
			assert_eq!(
				data,
				ChatCompletionMessage::SystemMessage { content: "world".to_string(), name: None }
			)
		}
	...
	fn test_assistants_message_tool_call_01_decode_fail() -> Result<()> {
		...
		.to_string();
		let data: serde_json::error::Result<AssistantToolCall> =
			serde_json::from_str(&fx_tool_call);
