...
use crate::data::graph::usage_stats::{ConnectionUsageStats, DeploymentUsageStats, MetricsUsageStats, PeriodStats, ProjectUsageStats, StatValue, VirtualKeyUsageStats};
use crate::data::project::ProjectId;
use crate::data::request_log::{pg_insert_m, RequestLogData};
use crate::data::virtual_key::VirtualKeyId;
use crate::errors::DataAccessError;


impl DataAccess {
    pub(crate) async fn load_virtual_key_usage_and_set_cache(&self, virtual_key_id: &VirtualKeyId, cached_stats_map: &BTreeMap<String, Option<String>>, now_utc: &DateTime<Utc>) -> Result<VirtualKeyUsageStats, DataAccessError>{
        let virtual_key_cached_stats = VirtualKeyUsageStats::extract_from_map(virtual_key_id, now_utc, cached_stats_map);
        ...
        let record: DbUsageStatsRecord = self.database.load_virtual_key_usage(virtual_key_id, now_utc).await?;
        ...
        match self.cache.set_usage_stats(&stats.0).await {
            Ok(_) => {println!("Successfully updated cached usage stats for {}", virtual_key_id);}
            Err(e) => {println!("Failed to update usage stats for {}: {:?}", virtual_key_id, e);}
        }

        Ok(stats)
    }

    pub(crate) async fn load_connection_usage_and_set_cache(&self, connection_id: &ConnectionId, cached_stats_map: &BTreeMap<String, Option<String>>, now_utc: &DateTime<Utc>) -> Result<ConnectionUsageStats, DataAccessError>{
        let connection_cached_stats = ConnectionUsageStats::extract_from_map(connection_id, now_utc, cached_stats_map);
        ...
        // If at least one of the usage stats is not cached we fetch it from our source of truth: The DB
        let record: DbUsageStatsRecord = self.database.load_connection_usage(connection_id, now_utc).await?;

        let stats = ConnectionUsageStats::from_db_record(connection_id, now_utc, record);
        match self.cache.set_usage_stats(&stats.0).await {
            Ok(_) => {println!("Successfully updated cached usage stats for {}", connection_id);}
            Err(e) => {println!("Failed to update usage stats for {}: {:?}", connection_id, e);}
        }

        Ok(stats)
    }

    pub(crate) async fn load_project_usage_and_set_cache(&self, project_id: &ProjectId, cached_stats_map: &BTreeMap<String, Option<String>>, now_utc: &DateTime<Utc>) -> Result<ProjectUsageStats, DataAccessError>{
        let project_cached_stats = ProjectUsageStats::extract_from_map(project_id, now_utc, cached_stats_map);
        ...
        // If at least one of the usage stats is not cached we fetch it from our source of truth: The DB
        let record: DbUsageStatsRecord = self.database.load_project_usage(project_id, now_utc).await?;

        let stats = ProjectUsageStats::from_db_record(project_id, now_utc, record);
        match self.cache.set_usage_stats(&stats.0).await {
            Ok(_) => {println!("Successfully updated cached usage stats for {}", project_id);}
            Err(e) => {println!("Failed to update usage stats for {}: {:?}", project_id, e);}
        }

        Ok(stats)
    }

    pub(crate) async fn load_deployment_usage_and_set_cache(&self, deployment_id: &DeploymentId, cached_stats_map: &BTreeMap<String, Option<String>>, now_utc: &DateTime<Utc>) -> Result<DeploymentUsageStats, DataAccessError>{
        let deployment_cached_stats = DeploymentUsageStats::extract_from_map(deployment_id, now_utc, cached_stats_map);
        ...
        // If at least one of the usage stats is not cached we fetch it from our source of truth: The DB
        let record: DbUsageStatsRecord = self.database.load_deployment_usage(deployment_id, now_utc).await?;

        let stats = DeploymentUsageStats::from_db_record(deployment_id, now_utc, record);
        match self.cache.set_usage_stats(&stats.0).await {
            Ok(_) => {println!("Successfully updated cached usage stats for {}", deployment_id);}
            Err(e) => {println!("Failed to update usage stats for {}: {:?}", deployment_id, e);}
        }
...
impl Database {
    pub(crate) async fn load_project_usage(&self, project_id: &ProjectId, now_utc: &DateTime<Utc>) -> Result<DbUsageStatsRecord, DataAccessError> {
        match self {
            Database::Postgres { pool } => {
                let mut query = pg_get_project_usage(project_id, now_utc);
                let sql = query.build_query_as::<DbUsageStatsRecord>();
                // TODO: Handle errors properly
                let result = sql.fetch_one(pool).await.map_err(|e| DatabaseError::SqlxError(e.to_string()))?;
    ...
    pub(crate) async fn load_virtual_key_usage(&self, virtual_key_id: &VirtualKeyId, now_utc: &DateTime<Utc>) -> Result<DbUsageStatsRecord, DataAccessError> {
        match self {
            Database::Postgres { pool } => {
    ...
    pub(crate) async fn load_deployment_usage(&self, deployment_id: &DeploymentId, now_utc: &DateTime<Utc>) -> Result<DbUsageStatsRecord, DataAccessError> {
        match self {
            Database::Postgres { pool } => {
    ...
    pub(crate) async fn load_connection_usage(&self, connection_id: &ConnectionId, now_utc: &DateTime<Utc>) -> Result<DbUsageStatsRecord, DataAccessError> {
        match self {
            Database::Postgres { pool } => {
                ...
                ExternalCache::Redis { connection } => {
                    ...
                    for (key, value) in stats_map {
                        match value {
                            StatValue::Int(v) => {
                                pipe
                                    .cmd("INCRBY").arg(&key).arg(v)
                                    .cmd("EXPIRE").arg(&key).arg(60);
                            }
                            StatValue::Float(v) => {
                                pipe
                                    .cmd("INCRBYFLOAT").arg(&key).arg(v)
                                    .cmd("EXPIRE").arg(&key).arg(60);
                            }
                            StatValue::NotSet => {
                    ...
                    pipe.exec_async(&mut conn)
                        .await
                        .map_err(|e| CacheError::RedisExecutionError { cause: e.to_string() })?;
                }
            }
        }
        ...
        |(mut req_acc, mut bud_acc, mut tok_acc), record| {
            ...
            req_acc.extend(
                ConnectionUsageStats::generate_request_keys_with_values(
                    ...
                    .into_iter()
                    .chain(
                        VirtualKeyUsageStats::generate_request_keys_with_values(
                    ...
                    .chain(
                        DeploymentUsageStats::generate_request_keys_with_values(
                    ...
                    .chain(
                        ProjectUsageStats::generate_request_keys_with_values(
            ...
            bud_acc.extend(
                ConnectionUsageStats::generate_budget_keys_with_values(
                    ...
                    .into_iter()
                    .chain(
                        VirtualKeyUsageStats::generate_budget_keys_with_values(
                            &record.graph.virtual_key.data.id,
                            &record.request_ts,
                            cost,
                        ),
                    ...
                    .chain(
                        DeploymentUsageStats::generate_budget_keys_with_values(
                            &record.graph.deployment.data.id,
                            &record.request_ts,
                            cost,
                        ),
                    ...
                    .chain(
                        ProjectUsageStats::generate_budget_keys_with_values(
                            &record.graph.project.data.id,
                            &record.request_ts,
                            cost,
                        ),
            ...
            tok_acc.extend(
                ConnectionUsageStats::generate_token_keys_with_values(
                    ...
                    .into_iter()
                    .chain(
                        VirtualKeyUsageStats::generate_token_keys_with_values(
                            &record.graph.virtual_key.data.id,
                            &record.request_ts,
                            tokens,
                        ),
                    ...
                    .chain(
                        DeploymentUsageStats::generate_token_keys_with_values(
                            &record.graph.deployment.data.id,
                            &record.request_ts,
                            tokens,
                        ),
                    ...
                    .chain(
                        ProjectUsageStats::generate_token_keys_with_values(
                            &record.graph.project.data.id,
                            &record.request_ts,
                            tokens,
                        ),
...
pub(crate) fn pg_get_connection_usage<'a>(connection_id: &'a ConnectionId, ts: &'a DateTime<Utc>) -> QueryBuilder<'a, Postgres> {
    let current_minute = Utc.with_ymd_and_hms(ts.year(), ts.month(), ts.day(), ts.hour(), ts.minute(), 0).unwrap();
    let current_hour = Utc.with_ymd_and_hms(ts.year(), ts.month(), ts.day(), ts.hour(), 0, 0).unwrap();
    let current_day = Utc.with_ymd_and_hms(ts.year(), ts.month(), ts.day(), 0, 0, 0).unwrap();
    let current_month = Utc.with_ymd_and_hms(ts.year(), ts.month(), 1, 0, 0, 0).unwrap();
...
pub(crate) fn pg_get_virtual_key_usage<'a>(virtual_key_id: &'a VirtualKeyId, ts: &'a DateTime<Utc>) -> QueryBuilder<'a, Postgres> {
    let current_minute = Utc.with_ymd_and_hms(ts.year(), ts.month(), ts.day(), ts.hour(), ts.minute(), 0).unwrap();
    let current_hour = Utc.with_ymd_and_hms(ts.year(), ts.month(), ts.day(), ts.hour(), 0, 0).unwrap();
    let current_day = Utc.with_ymd_and_hms(ts.year(), ts.month(), ts.day(), 0, 0, 0).unwrap();
    let current_month = Utc.with_ymd_and_hms(ts.year(), ts.month(), 1, 0, 0, 0).unwrap();
...
pub(crate) fn pg_get_project_usage<'a>(project_id: &'a ProjectId, ts: &'a DateTime<Utc>) -> QueryBuilder<'a, Postgres> {
    let current_minute = Utc.with_ymd_and_hms(ts.year(), ts.month(), ts.day(), ts.hour(), ts.minute(), 0).unwrap();
    let current_hour = Utc.with_ymd_and_hms(ts.year(), ts.month(), ts.day(), ts.hour(), 0, 0).unwrap();
    let current_day = Utc.with_ymd_and_hms(ts.year(), ts.month(), ts.day(), 0, 0, 0).unwrap();
    let current_month = Utc.with_ymd_and_hms(ts.year(), ts.month(), 1, 0, 0, 0).unwrap();
...
pub(crate) fn pg_get_deployment_usage<'a>(deployment_id: &'a DeploymentId, ts: &'a DateTime<Utc>) -> QueryBuilder<'a, Postgres> {
    let current_minute = Utc.with_ymd_and_hms(ts.year(), ts.month(), ts.day(), ts.hour(), ts.minute(), 0).unwrap();
    let current_hour = Utc.with_ymd_and_hms(ts.year(), ts.month(), ts.day(), ts.hour(), 0, 0).unwrap();
    let current_day = Utc.with_ymd_and_hms(ts.year(), ts.month(), ts.day(), 0, 0, 0).unwrap();
    let current_month = Utc.with_ymd_and_hms(ts.year(), ts.month(), 1, 0, 0, 0).unwrap();
