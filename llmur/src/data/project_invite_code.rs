...
impl DataAccess {
    ...
    pub async fn create_invite_code(&self, project_id: &ProjectId, assign_role: &ProjectRole, validity: &Option<String>, code_length: &Option<usize>) -> Result<ProjectInviteCode, DataAccessError> {
        ...
        let valid_until: Option<DateTime<Utc>> = if let Some(validity) = validity {
            let seconds = parse_and_add_to_current_ts(&validity).map_err(|_| DataAccessError::InvalidInviteCode)? ; //TODO
            Some(
                chrono::DateTime::from_timestamp(seconds, 0).ok_or(DataAccessError::InvalidInviteCode)?
            )
        } else { None };
...
impl ConvertInto<ProjectInviteCode> for DbProjectInviteCodeRecord {
    fn convert(self, _application_secret: &Option<Uuid>) -> Result<ProjectInviteCode, DataConversionError> {
