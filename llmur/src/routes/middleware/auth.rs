...
pub type AuthorizationHeaderExtractionResult =
    Result<AuthorizationHeader, Arc<AuthenticationError>>;
...
pub(crate) fn auth_token_extraction_mw(
    ...
    Box::pin(async move {
        ...
        let result: AuthorizationHeaderExtractionResult = match headers.get("Authorization") {
            None => Err(Arc::new(AuthenticationError::AuthHeaderNotProvided)),
            Some(auth_header) => extract_bearer_token(auth_header)
