...
use crate::routes::responses::responses_route;
...
pub(crate) fn admin_routes(state: Arc<LLMurState>) -> Router<Arc<LLMurState>> {
    Router::new()
        ...
        .nest(
            "/project-invite-code",
            project_invite_code::routes(state.clone()),
        )
...
pub(crate) fn openai_v1_routes(state: Arc<LLMurState>) -> Router<Arc<LLMurState>> {
    Router::new()
        ...
        .route(
            "/chat/completions",
            post(chat_completions_route).route_layer(from_fn_with_state(
                state.clone(),
                openai_route_controller_mw::<
        ...
        .route(
            "/responses",
            post(responses_route).route_layer(from_fn_with_state(
                state.clone(),
                openai_route_controller_mw::<
                    crate::providers::openai::responses::request::Request,
                    crate::providers::openai::responses::response::Response,
                >,
            )),
        )
        .with_state(state.clone())
